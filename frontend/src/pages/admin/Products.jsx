...
export default function AdminProducts() {
  ...
  const openEdit   = (p) => {
    setForm({ productName:p.productName, productDescription:p.productDescription, productPrice:p.productPrice,
      productCategory:p.productCategory, productStatus:p.productStatus, productQuantity:p.productQuantity,
  ...
  const handleSave = async (e) => {
    ...
    const { ok, data } = modal === 'create'
      ...
      : await api.patchForm(`/products/product/${editId}`, fd)
    ...
    <div className="page-wrap">
      <div className="flex items-center justify-between mb-8 gap-4 flex-wrap">
        <div>
          ...
          <p className="text-slate mt-1">{products.length} total products</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <div className="relative flex-1 max-w-sm">
          ...
          <input className="input-field pl-11 rounded-full" placeholder="Search products..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
            ...
            <div key={p._id} className="card overflow-hidden group hover:shadow-card-lg transition-all">
              <div className="relative aspect-[4/3] bg-pink-50 overflow-hidden">
                ...
                <div className="absolute inset-0 bg-ink/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                  <button onClick={() => openEdit(p)} className="w-9 h-9 rounded-xl bg-white text-ink flex items-center justify-center hover:text-pink transition-colors shadow-card">
                    <Pencil size={14}/>
                  </button>
                ...
                <div className="absolute top-2 left-2 flex gap-1">
                  <span className={`badge ${p.productStatus === 'available' ? 'badge-green' : 'badge-gray'} text-[9px]`}>{p.productStatus}</span>
                </div>
                {p.restaurant?.name && (
              ...
              <div className="p-3">
                ...
                <p className="text-muted text-[10px] mt-0.5">Qty: {p.productQuantity}</p>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="bg-white rounded-4xl shadow-float w-full max-w-lg max-h-[92vh] overflow-y-auto animate-scale-in" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div>
                ...
                <label className="flex items-center gap-3 w-full bg-pink-50 border-2 border-dashed border-pink-200 rounded-2xl px-4 py-4 cursor-pointer hover:border-pink hover:bg-pink-100 transition-colors group">
